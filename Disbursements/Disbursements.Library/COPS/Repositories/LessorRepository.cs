...
namespace Disbursements.Library.COPS.Repositories
{
    public class LessorRepository
    {
        ...
        private PaymentLessorView GetPaymentData(PaymentLessorView payment)
        {
            var output = new PaymentLessorView();
            using (IDbConnection cn = new SqlConnection(server.SAP_DISBURSEMENTS))
            {
                var opdata = new  List<PaymentLessorHeaderView> { payment.Header};
                using (var multi = cn.QueryMultiple
                (
                    "spLessorPayment",
                    new
                    {
                        mode = "GET_LESSOR_PAYMENT_DATA",
                        opdata = opdata.ToDataTable(),
                        accountdata = payment.Accounts.ToDataTable(),
                    }, commandType: CommandType.StoredProcedure, commandTimeout: 0)
                )
                {
                    output.Header = multi.ReadFirst<PaymentLessorHeaderView>();
                    output.Accounts = multi.Read<PaymentLessorAccountView>();
        ...
        //public void PostLessorPayment(PaymentLessorView payment)
        //{
